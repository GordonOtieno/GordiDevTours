...
var displayMap = function displayMap(locations) {
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); //from the model
    //add pop ups
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'password' ? 'http://127.0.0.1:8000/api/v1/users/updateMyPassword' : 'http://127.0.0.1:8000/api/v1/users/updateMyProfile';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
...
var stripe = Stripe('pk_test_exK8rRRqiRKAibrRdGNvIXFh00U0z9Ajcl');
...
if (userdata) userdata.addEventListener('submit', function (e) {
  e.preventDefault();
  var form = new FormData(); //this is a multipart form
...
if (userpassword) userpassword.addEventListener('submit', /*#__PURE__*/function () {
